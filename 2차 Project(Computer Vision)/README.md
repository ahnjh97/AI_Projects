...
PreDect는 보행 환경의 영상에서 **보행자 신호등의 빨간불과 녹색불**을 판별하고 **사람, 차량, 공 등**을 탐지하여 결과를 웹 서비스로 전달하는 프로젝트입니다. YOLO11n 학습부터 라벨 검수, OpenVINO 변환, Flask API까지 연결했습니다.

특히 원거리 보행자 신호등이 누락되는 문제를 분석하고, **고해상도 원본 데이터 확보와 자동 라벨 후보 생성과 사람 검수**를 통해 학습 데이터를 개선했습니다.
...
- **Detection**: Ultralytics YOLO11n (보행자 신호등 판별과 주변 객체 탐지)
...
- 전체 mAP50: 0.813
- 전체 mAP50–95: 0.547
- 보행자 신호등 녹색불 `ped_g` mAP50: 0.900
- 보행자 신호등 빨간불 `ped_r` mAP50: 0.912

`0.912`는 보행자 신호등 빨간불 클래스의 mAP50이며 전체 모델의 정확도가 아닙니다. 데이터 구성에는 `traffic_light`를 포함한 6개 클래스가 사용되지만, 저장된 최종 검증 로그에 개별 지표가 표시된 클래스는 5개입니다. `traffic_light` 성능을 별도로 입증한 수치로 해석하지 않습니다.
...
└─ notebooks/
   ├─ labeler.ipynb        # 보행자 신호등 Labeler 학습
